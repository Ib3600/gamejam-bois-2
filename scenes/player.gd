...
var cold_timer := 0.0
var heat_regen_rate := 10

@export var cold_damage_interval: float = 3.0
@export var cold_damage_amount: int = 10
var cold_damage_timer: float = 0.0
var _was_above_zero_heat: bool = true
...
func _physics_process(delta):
	...
	if is_knocked_back:
		...
		# ✅ Blocage du mouvement pendant attaque ou dégâts
...
func handle_heat(delta: float) -> void:
	...
	if is_cold:
		var perte_par_sec: float = max(0.0, base_cold_loss - Global.niveau_manteau)
		Global.player_heat -= perte_par_sec * delta
	elif Global.player_heat < 100.0:
...
# --- Attaque ---
func attack():
	if Global.dialogue or is_dead or is_attacking:
		return
...
func get_cold():
	...
	is_cold = data and data.has_custom_data("cold") and data.get_custom_data("cold")


# --- Knockback ---
func apply_knockback(source_position: Vector2):
	var direction = (global_position - source_position).normalized()
	knockback_velocity = direction * (knockback_force * get_physics_process_delta_time())
...
func play_damage_animation():
	if anim.sprite_frames.has_animation("take_damage"):
...
func die():
	...
	Textbox.queue_text("Les habitants du chalet vous suivront bientôt.")
